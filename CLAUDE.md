...
- v1.7.1: `Progress.status`/`phase` are `assets::JobStatus`/`JobPhase`, not `String`s set by literal at fifteen sites; `provisioning::Incomplete.kind` is `IncompleteKind`; `Config.on_name_collision` is `NameCollision`. **The serialized names are unchanged** (`docs/UI.md` documents the incomplete kinds and the frontend renders the job status), and a serialization test per enum is what says so. `NameCollision` uses `#[serde(other)]` on `Suffix` deliberately: the old code compared case-insensitively against `"error"` and treated anything else as "suffix", so a typo in a config file kept working — and since Phase 1 a config that will not parse stops every command. The `config set` path stays strict, because a typo typed at the command line is a mistake to report.
- v1.7.1: `assets::AssetEntry` carries **both** `rel: String` (textual, lossy, what globs / `SafeRelativePath` / the browser JSON reason about) and `os_rel: PathBuf` (exact, what you join to open or create a file). Never join `rel`. A template file whose name is not valid UTF-8 used to be opened at a `?`-substituted path that does not exist, so the create failed naming a path the user never wrote. `assets::interp_rel_os` builds the destination component by component and only converts a component to `str` when it contains `{`, so a name with no token passes through byte-exact. Validation stays on the textual form — `..`, drive letters and reserved device names are all ASCII.
- v1.7.1: every recursive walk stops at `util::paths::MAX_WALK_DEPTH` (256) and reports through `paths::too_deep`. `tree_size` turns it into `None` like any other read failure; `fs_retry::clear_readonly_tree` just stops descending, because it is best-effort and the caller is about to try the delete anyway.
- v1.7.1: a path that will be **stored** goes through `util::paths::storable`, which refuses rather than mangling. `display().to_string()` substitutes `?` for non-UTF-8 bytes, and writing that into `config.toml` records a directory that does not exist; TOML cannot hold the bytes either way, so the honest answers are "store it" and "say why not at the moment it is set".
