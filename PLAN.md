...
2. `git checkout main && git pull`, then branch `phase-NN-<slug>`. Run the baseline gates (below) and confirm green before changing anything. **The previous phase's PR must already be merged** — if it is not, stop and say so rather than branching off it. Phases 1-3 stacked three deep because this was assumed and never written down, and unpicking a stack is worse than waiting for a merge.
...
7. **Merge your PR before the next phase starts.** One open phase PR at a time, always based on `main`. Two notes for whoever does the merging: a stacked PR needs a **merge commit**, not a squash (squashing gives the commits new ids while the branch above still carries the originals, which manufactures conflicts); and never pass `--delete-branch` while another PR is based on that branch, because deleting a base branch closes the PR above it and it cannot be reopened until the branch is restored.
8. Release checkpoints are separate sessions that use the `release` skill. A phase never bumps the version or tags.
...
**Steps.**
- [x] Rewire or rewrite the affected tests first (`windows_semantics.rs`, `integration.rs:2410`, `ui_server.rs:967`, `hostile_fs.rs:194`), keeping the guarantees they express.
- [x] Deletions and narrowings as listed; `cargo clippy` and `cargo doc` clean.
- [x] `CLAUDE.md` corrections; `ROADMAP.md` phase line.
...
**Notes.**
- `ReconcileReport.swept` **kept** as a serialized field: `docs/UI.md` documents it as part of the `/api/reconcile` response ("remains for response compatibility and is always zero"), and the ground rules keep HTTP JSON shapes compatible. Its dead readers are gone — the `is_empty` term and the `cli/reconcile.rs` print branch.
- `fs_retry::codes::ERROR_DIR_NOT_EMPTY`/`ERROR_LOCK_VIOLATION` were left `pub`: the `codes` module is itself private, so they were never externally reachable, and narrowing two of its four constants only made the module inconsistent.
- Debug test count 408 → 402. `assets.rs` lost seven unit tests and gained one (`walk` must still report links, which is what keeps template copying's warning honest); `transactions.rs` gained three that carry the deleted move guarantees (link refusal, missing-source error, short/dropped-file verification) plus a literal-braces assertion; `naming.rs` lost the three tests of the deleted `ensure_relative_safe_path` delegate, whose cases now live in `validated.rs`'s existing `SafeRelativePath` tests.
- `windows_semantics.rs` kept the unlocked entry points (renamed to `*_unlocked`) rather than switching to `*_configured`: that suite has no `FASTF_INSTALL_DIR`/`HOME` sandbox, so the configured variants would load and lock the developer's real data directory.
- `library::move_project_with_outcome` was deleted too (it had exactly one caller once the other three wrappers went), so `move_project` now holds the lock, revalidates, and calls `move_project_unlocked` directly.
...
| A release | 2026-08-23 | [#13](https://github.com/cristocola/fast-folder/pull/13) | The v1.6.1 checkpoint found one Phase 4 defect that only the packaging build could see: `tests/repo_hygiene.rs` skipped on "no git checkout", but the AUR source package unpacks the release tarball into an **ignored directory inside a real clone**, where `git ls-files` succeeds and returns nothing — the vacuous-pass assertion then failed `check()` for anyone building `fast-folder` from the AUR. The guard is now "the crate directory is the checkout's top level". Lesson for later tracks: `makepkg -f` in `packaging/aur/fast-folder/` is a gate the seven local gates do not cover. |
| 5 | 2026-08-23 | [#11](https://github.com/cristocola/fast-folder/pull/11) | Track A complete. `util::paths::require_real_file`/`require_native_relative` are the shared boundary checks; `MoveManifest::scan`/`verify_destination` hold every move invariant (`assets` no longer has a move engine); a `pub` mutator without `DataLock` is named `*_unlocked` and has a `*_configured` twin plus a private `*_inner` body; `reconcile_locked()` takes no argument; the pre-v2 marker writers are gone, so tests plant those bytes themselves. |
