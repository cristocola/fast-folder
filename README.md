...
- **Opens projects in the tools you already use.** Reveal a folder in your file manager, open a terminal inside it, put its path on the clipboard, print the path on stdout for `cd "$(fastf path api)"`, or `fastf cd api` straight from the shell. Bind fast-folder to a hotkey on your desktop and it opens a terminal for itself when a question needs an answer.
